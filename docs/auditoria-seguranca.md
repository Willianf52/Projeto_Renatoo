...
| A07 Falhas de autenticação | RLS bem desenhado (bloqueia por `usuario_ativo()`, escopo por `cargo`), redirect pós-login validado por `safeRedirectPath` (agora com teste automatizado). ~~**Gap:** a troca de senha não pede mais a senha atual.~~ Corrigido — `trocar-senha/page.tsx` reautentica com `signInWithPassword` contra a senha atual antes de chamar `updateUser`; falhando a reautenticação, a troca nem é tentada. **Correção de registro (2026-08-28):** esta linha esteve errada entre a data em que foi escrita e 28/08. A reautenticação estava descrita aqui como feita, mas não existia no código — `git log -S'signInWithPassword'` sobre o arquivo não devolvia commit nenhum, e a tela só coletava a nova senha e a confirmação. Era o achado A-1 da auditoria de AppSec de 28/08, e foi nessa data que o campo "Senha atual" e a reautenticação de fato entraram. Um documento que declara um controle como pronto é o que impede alguém de reauditá-lo; a lição fica junto com a correção. ~~**Gap:** login não dá nenhum feedback de limitação de tentativas.~~ Corrigido — bloqueio de 30s após 5 tentativas em `components/LoginForm.tsx`. | `components/LoginForm.tsx` (era `LoginPage.tsx`), `cadastros/trocar-senha/page.tsx` |
...
- **"Require current password when updating" está desligado** (achado novo, da
  ...
  `updateUser` — **desde 2026-08-28, e não antes**; até ali esta frase descrevia
  uma intenção, não o código (achado A-1 da auditoria de AppSec daquele dia).
  Mas isso é do lado do cliente — uma sessão roubada refaz o
