...
| CI não rodava em push para `main` | Gatilho apontado para `branches: [main]`. O required status check "build" já estava marcado na proteção de `cursor/login-page-performance-lab`, mas `enforce_admins` estava `false` — administrador do repositório (papel de quem empurra direto) pulava a checagem obrigatória, confirmado num push real em 2026-08-21 ("Bypassed rule violations"). Ativado via `gh api ... /protection/enforce_admins` no mesmo dia: nenhum push direto entra mais nessa branch sem o job já ter passado para aquele commit, nem de admin |
