 ...
 *
 * Duas fronteiras aqui, e nao tres: esta tela nao tem barra de acoes, e o
 * titulo nao depende de consulta nenhuma -- fica inteiro no shell estatico.
 */
export default function RankingDeInspecoesPage({
  searchParams,
}: {
 ...
 * Inicial/Final estreitas + Checkpoint flexivel, linha 2 com o resto dos
 * selects e o Filtrar como ultima celula -- como na referencia (Filtrar nao
 * ocupa a linha inteira aqui, diferente do Registro de Rondas).
 */
...
async function CorpoDoRanking({ searchParams }: { searchParams: SearchParamsPromise }) {
  const filtros = extrairFiltros(await searchParams);
  const ranking = await getRankingDeInspecoes(filtros);
