...
// `montarHierarquiaDeResponsaveis` morava aqui; virou `montarHierarquiaDePerfis`
// em `lib/hierarquia-de-perfis.ts` quando Grupo de Usuarios passou a precisar
// do mesmo algoritmo para o campo "Usuários". Reexportada com o nome antigo
// para nao obrigar quem ja importa daqui a mudar.
export { montarHierarquiaDePerfis as montarHierarquiaDeResponsaveis } from "@/lib/hierarquia-de-perfis";
...
export async function getOpcoes(): Promise<{
  ...
  const [grupos, tipos, perfis] = await Promise.all([
    ...
    //
    // Ordenado por nome ja aqui: `montarHierarquiaDePerfis` preserva a
  ...
  return {
    ...
    responsaveis: montarHierarquiaDePerfis(perfis.data ?? []),
  };
}
